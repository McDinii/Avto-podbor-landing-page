  ...
  if (!empty($_POST['name']) && !empty($_POST['phone'])) {
    ...
    // Имя
        ...
        if (isset($_POST['email']) && !empty($_POST['email'])) {
            $txt .= "Почта: " . strip_tags(trim(urlencode($_POST['email']))) . "%0A";
        }
        // Телеграм
        if (isset($_POST['tg']) && !empty($_POST['tg'])) {
            $txt .= "Telegram: " . strip_tags(trim(urlencode($_POST['tg']))) . "%0A";
        }
        // Способ связи
        if (isset($_POST['radio']) && !empty($_POST['radio'])) {
             $txt .= "Желаемый способ связи: " . strip_tags(trim(urlencode($_POST['radio']))) . "%0A";
        }
    ...
    if( isset(json_decode($textSendStatus)->{'ok'}) && json_decode($textSendStatus)->{'ok'} ) {
      if (!empty($_FILES['files']['tmp_name'])) {
          ...
          $path = $_SERVER['DOCUMENT_ROOT'] . 'botmessage/tmp/';
