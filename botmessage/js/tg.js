(function ($) {
    $(".form-script").submit(function (event) {
        event.preventDefault();

        // Сообщения формы
        let successSendText_1 = "С вами свяжутся в течениии дня";
        let successSendText_2 = "С вами свяжутся в течениии 60 минут";
        let errorSendText = "Сообщение не отправлено. Попробуйте еще раз!";
        let requiredFieldsText = "Заполните поля с именем и телефоном";

        // Сохраняем в переменную класс с параграфом для вывода сообщений об отправке
        let message = $(this).find(".cta-form__message");


        let id = $(this).attr("id")
        let form = $("#" + $(this).attr("id"))[0];
        let fd = new FormData(form);
        $.ajax({
            url: "botmessage/php/tgm.php",
            type: "POST",
            data: fd,
            processData: false,
            contentType: false,
            beforeSend: () => {
                $(".preloader").addClass("preloader_active");
            },
            success: function success(res) {
                $(".preloader").removeClass("preloader_active");
                // Посмотреть на статус ответа, если ошибка
                // console.log(res);
                let respond = $.parseJSON(res);
                if (respond === "SUCCESS") {
                    $('#modal-check').fadeIn();
                    if (id !== "contact-form_2"){
                    message.text(successSendText_1).css("color", "#438BAD");}
                    else{
                        message.text(successSendText_2).css("color", "#438BAD");
                    }
                    setTimeout(() => {
                        message.text("");
                    }, 10000);
                } else if (respond === "NOTVALID") {
                    message.text(requiredFieldsText).css("color", "#d42121");
                    setTimeout(() => {
                        message.text("");
                    }, 4000);
                } else {
                    message.text(errorSendText).css("color", "#d42121");
                    setTimeout(() => {
                        message.text("");
                    }, 4000);
                }
            }
        });
    });
})(jQuery);
